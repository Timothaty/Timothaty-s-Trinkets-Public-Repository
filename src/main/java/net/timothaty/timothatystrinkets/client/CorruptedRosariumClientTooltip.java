...
@OnlyIn(Dist.CLIENT)
public class CorruptedRosariumClientTooltip implements ClientTooltipComponent {
	...
	private static final int NAME_COLOR = 0xE6001F;
	private static final int NAME_HEIGHT = 9;
	private static final int NAME_SPACING = 2;
	private static final float[][] SECTION_COLORS = {
		{0x3C / 255.0F, 0x39 / 255.0F, 0x47 / 255.0F},
		{0x92 / 255.0F, 0x04 / 255.0F, 0x17 / 255.0F}
	};
	private static final int BACKGROUND_WIDTH =
			CorruptedRosariumData.SLOT_COUNT * SLOT_WIDTH + BORDER_WIDTH * 2;
	...
	public CorruptedRosariumClientTooltip(CorruptedRosariumTooltip tooltip) {
	...
	public void renderImage(Font font, int x, int y, GuiGraphics guiGraphics) {
		...
		for (int index = 0; index < CorruptedRosariumData.SLOT_COUNT; index++) {
	...
	private static void renderColoredBackground(GuiGraphics guiGraphics, int x, int y) {
		int sectionWidth = BACKGROUND_WIDTH / CorruptedRosariumData.SLOT_COUNT;
		for (int index = 0; index < CorruptedRosariumData.SLOT_COUNT; index++) {
