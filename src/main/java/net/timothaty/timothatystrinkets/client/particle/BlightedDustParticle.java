...
@OnlyIn(Dist.CLIENT)
public class BlightedDustParticle extends TextureSheetParticle {
	private final SpriteSet sprites;
	private final float baseSize;

	public static BlightedDustParticleProvider provider(SpriteSet spriteSet) {
		return new BlightedDustParticleProvider(spriteSet);
	}

	protected BlightedDustParticle(ClientLevel level, double x, double y, double z,
										   double xd, double yd, double zd, SpriteSet spriteSet) {
		super(level, x, y, z);

		this.sprites = spriteSet;

		this.baseSize = 0.2F + this.random.nextFloat() * 0.06F;
	...
	public static class BlightedDustParticleProvider implements ParticleProvider<SimpleParticleType> {
		private final SpriteSet spriteSet;

		public BlightedDustParticleProvider(SpriteSet spriteSet) {
		...
		public Particle createParticle(SimpleParticleType type, ClientLevel level,
			...
			return new BlightedDustParticle(level, x, y, z, xd, yd, zd, this.spriteSet);
		}
	}
}
