...
@OnlyIn(Dist.CLIENT)
public class StunnedSpiralParticle extends TextureSheetParticle {
	private static final Map<Integer, StunnedSpiralParticle> ACTIVE_PARTICLES = new HashMap<>();
	...
	public static StunnedSpiralParticleProvider provider(SpriteSet spriteSet) {
		return new StunnedSpiralParticleProvider(spriteSet);
	}

	public static class StunnedSpiralParticleProvider implements ParticleProvider<SimpleParticleType> {
		private final SpriteSet spriteSet;

		public StunnedSpiralParticleProvider(SpriteSet spriteSet) {
		...
		public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
			...
			if (entityId > 0) {
				...
				StunnedSpiralParticle active = ACTIVE_PARTICLES.get(entityId);
				...
				StunnedClientAnimationState.markStunned(entityId, worldIn);
				if (active != null && active.isStillAlive()) {
					active.refreshFromServerPing(x, y, z, ySpeed);
					return null;
				}
			}
			return new StunnedSpiralParticle(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
		}
	}
	...
	protected StunnedSpiralParticle(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
		super(world, x, y, z);
		this.spriteSet = spriteSet;
		...
		if (this.attachedEntityId > 0) {
			...
			StunnedClientAnimationState.markStunned(this.attachedEntityId, world);
		}
	...
	private void refreshFromServerPing(double x, double y, double z, double yOffset) {
		...
		StunnedClientAnimationState.markStunned(this.attachedEntityId, this.level);
	...
	public void remove() {
		super.remove();
		StunnedSpiralParticle active = ACTIVE_PARTICLES.get(this.attachedEntityId);
		if (active == this) {
